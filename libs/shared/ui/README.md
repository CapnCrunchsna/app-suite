...
§2.2 gives this lib the tightest dependency rule in the workspace — `type:ui` may
depend only on `type:ui` and `type:format` — so nothing here may import a domain
...
- Formatting is **not** here — see `libs/shared/format` (`@metrum/format`). It is a
  separate lib because `tools/parse-statement.mjs` is a Node CLI that needs the same
  money formatter, and this package resolves to an Angular bundle.
