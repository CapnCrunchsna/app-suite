/**
 * Ledgerline's palette — that app's default, and offered in every other.
 *
 * It lived in `apps/ledgerline-ui/` until 2026-09-11. It moved here when Edgeline
 * got an identity of its own and the suite settled on the arrangement
 * `SUITE_THEMES` describes: every app defaults to its own theme and offers all of
 * them. An app cannot import another app, so a palette that more than one app
 * offers has to live in the lib they share. It is pure data with no dependency
 * beyond `Theme`, so this costs `type:ui` nothing under §2.2.
 *
 * ## What it is a theme *of*
 *
 * A ledger. Not "finance" in the abstract — the specific physical object that app
 ...
 * `theming.spec.ts`. The light half in particular is not eyeballed: `#46d492`
