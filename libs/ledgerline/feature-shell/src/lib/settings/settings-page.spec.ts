...
class ApiStub {
  ...
  // statement produced, because it is the case the section exists for.
  ...
  mergeMerchant(id: string, body: MergeMerchantBody): Promise<MerchantMergeResult> {
    ...
    // it has run — which is exactly what the page now waits for.
    this.reviewQueue = {
      ...this.reviewQueue,
      mergeCandidates: [],
    };
...
describe('SettingsPage', () => {
  ...
  describe('merchants', () => {
    ...
    it('flips the direction without merging anything', async () => {
      ...
      expect(api.merges).toEqual([]);
      expect(el.querySelector('.button--primary')?.textContent).toContain(
        'Merge into SAMS CLUB',
      );
    });
