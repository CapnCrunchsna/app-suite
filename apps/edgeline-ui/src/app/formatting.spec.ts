 ...
 *
 * Three failures this file exists to catch, all of which render *something*
 ...
 *    identically (§12's reason for returning `null` at all).
 * 3. A 0–1 hit rate run through a percentage formatter — 55% shown as 0.55%.
 */
...
describe('money is rendered from cents (§1, §11.2)', () => {
  ...
  it('round-trips a form field through dollars without float drift', () => {
    ...
    // 0.1 + 0.2 territory: 8.29 * 100 is 828.9999… in binary floating point.
...
describe('percentages keep null apart from zero (§12)', () => {
  it('formats an already-percentage value', () => {
    expect(formatPercent(2.5)).toBe('2.50%');
    expect(formatPercent(0)).toBe('0.00%');
  });

  it('signs CLV, where negative is the finding rather than an error', () => {
  ...
  it('never turns "nothing has settled" into "you lose every bet"', () => {
    ...
    // The distinction the API draws on purpose — a real zero still renders.
    expect(formatRatioAsPercent(0)).toBe('0.0%');
  });

  it('scales hit rate from a ratio, not from a percentage', () => {
...
describe('odds convert only at the display edge (§1)', () => {
  it('turns decimal into American on both sides of even money', () => {
    ...
    expect(toAmerican(1.909)).toBe('-110');
  });
...
describe('times render in the reader’s timezone (§1, §11.2)', () => {
  ...
  it('shows the local clock reading for the instant, not the UTC one', () => {
    ...
    // pass in UTC — and would be the exact bug §1 is guarding against. Both
    // clock conventions are accepted because the locale is the machine's too.
    const local = new Date(iso);
    const twentyFour = String(local.getHours());
    const twelve = String(local.getHours() % 12 || 12);
    const clock = formatLocalClock(iso);
    expect(clock.includes(twelve) || clock.includes(twentyFour)).toBe(true);
    expect(formatLocalDay(iso)).toContain(String(local.getDate()));
  });

  it('renders a missing timestamp as no data', () => {
...
describe('heartbeat age (§13)', () => {
  const now = Date.parse('2026-09-08T12:00:00Z');

  it('reads as an age, so a dead worker is obvious without arithmetic', () => {
  ...
  it('calls a heartbeat stale past the window, and absent when there is none', () => {
...
describe('“today” is the reader’s day (§11.2)', () => {
